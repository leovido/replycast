...
function authorHasReplied(subtree: any, authorFid: number): boolean {
  ...
  if (subtree.replies && Array.isArray(subtree.replies)) {
    return subtree.replies.some((child: any) => authorHasReplied(child, authorFid));
  }
  return false;
}

// Collect all replies where the author has not replied in the sub-thread
function collectUnrepliedReplies(subtree: any, authorFid: number, parentIsAuthor: boolean): any[] {
  ...
  for (const reply of subtree.replies) {
    ...
    unreplied = unreplied.concat(collectUnrepliedReplies(reply, authorFid, false));
  }
  return unreplied;
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!authorFid) {
      return res.status(400).json({ error: 'Could not determine author FID from root cast' });
    }
    const unrepliedReplies = collectUnrepliedReplies(conversation, authorFid, true);
    ...
    return res.status(500).json({ error: 'Failed to fetch unreplied replies', message: error instanceof Error ? error.message : String(error) });
  }
} 